  ...
  <div class="addEdit-block">
    ...
    >
      <el-row >
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="comment oncontent" prop="content">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="comment oncontent"
                  v-model="ruleForm.content" readonly>
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.content" label="comment oncontent" prop="content">
...
export default {
  data() {
	...
	reply : false,
      ...
      rules: {
          ...
          content: [
                { required: true, message: 'Comment oncontent cannot be empty', trigger: 'blur' },
          ],
          reply: [
          ],
      }
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `discussjiaotongxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `discussjiaotongxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.discussjiaotongxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
